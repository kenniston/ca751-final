...
void SmartAppLayer::initialize(int stage)
{
...
simtime_t SmartAppLayer::computeAsynchronousSendingTime(simtime_t interval, ChannelType chan)
{
    /*
...
void SmartAppLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial) {
...
void SmartAppLayer::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
{
    Enter_Method_Silent();
...
void SmartAppLayer::handlePositionUpdate(cObject* obj)
{
...
void SmartAppLayer::handleParkingUpdate(cObject* obj)
{
    isParked = mobility->getParkingState();
}

void SmartAppLayer::handleLowerMsg(cMessage* msg)
{
...
void SmartAppLayer::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind()) {
...
void SmartAppLayer::startService(Channel channel, int serviceId, std::string serviceDescription)
{
...
void SmartAppLayer::stopService()
{
    cancelEvent(sendWSAEvt);
    currentOfferedServiceId = -1;
}

void SmartAppLayer::sendDown(cMessage* msg)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDown(msg);
}

void SmartAppLayer::sendDelayedDown(cMessage* msg, simtime_t delay)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDelayedDown(msg, delay);
}

void SmartAppLayer::checkAndTrackPacket(cMessage* msg)
{
