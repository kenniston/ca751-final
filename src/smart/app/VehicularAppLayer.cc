...
namespace veins {
    ...
    void VehicularAppLayer::initialize(int stage)
    {
    ...
    simtime_t VehicularAppLayer::computeAsynchronousSendingTime(simtime_t interval, ChannelType chan)
    {
        /*
    ...
    void VehicularAppLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial) {
    ...
    void VehicularAppLayer::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
    {
        Enter_Method_Silent();
    ...
    void VehicularAppLayer::handlePositionUpdate(cObject* obj)
    {
    ...
    VehicularAppLayer::~VehicularAppLayer()
    {
    ...
    void VehicularAppLayer::startService(Channel channel, int serviceId, std::string serviceDescription)
    {
    ...
    void VehicularAppLayer::stopService()
    {
        cancelEvent(sendWSAEvt);
        currentOfferedServiceId = -1;
    }

    void VehicularAppLayer::sendDown(cMessage* msg)
    {
        checkAndTrackPacket(msg);
        BaseApplLayer::sendDown(msg);
    }

    void VehicularAppLayer::sendDelayedDown(cMessage* msg, simtime_t delay)
    {
        checkAndTrackPacket(msg);
        BaseApplLayer::sendDelayedDown(msg, delay);
    }

    void VehicularAppLayer::checkAndTrackPacket(cMessage* msg)
    {
        if (dynamic_cast<BasicSafetyMessage*>(msg)) {
            EV_TRACE << "sending down a BSM" << std::endl;
            generatedBSMs++;
        }
        else if (dynamic_cast<ServiceAdvertisment*>(msg)) {
